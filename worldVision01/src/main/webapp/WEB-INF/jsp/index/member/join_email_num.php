...
$db->Query("select * from BlueAD_member where member_email='$member_email' ");
$row = $db->Fetch();

if($row){
	echo "<script language='javascript'>";
	echo "alert('이미 회원가입이 되어있습니다.');";
...
$db->Query(" insert into BlueAD_member_num (member_name, member_email, member_num, wdate) values ('$member_name','$member_email','$coupon_type','$bbs_date')  ");
	...
	$m_contents .= 				"             <td bgcolor='#f7f7f7' align='center' colspan='2'>회원가입 인증번호</td>";
	$m_contents .= 				"           </tr>        ";
	$m_contents .= 				"         <tr height='30'>";
	...
	$m_contents .= 				"          </tr>";
	$m_contents .= 				"           <tr height='30'>";
	$m_contents .= 				"             <td bgcolor='#f7f7f7' align='center' colspan='2'>월드비전선명회합창단 <a href='http://".$_SERVER['HTTP_HOST']."' target='_blank'>http://".$_SERVER['HTTP_HOST']."</a></td>";
	$m_contents .= 				"           </tr>        ";
	$m_contents .= 				" 				</table>";
	...
	$mail_subject="[월드비전선명회합창단]회원가입 인증번호입니다..";
	...
	$aa = mail($to, $mail_subject, $mail_contents, $additional_headers);	

?>
<script type="text/javascript">
	alert("인증번호 메일발송되었습니다.");
	parent.document.form.member_email_num.value = '<?=$coupon_type;?>';
</script>
